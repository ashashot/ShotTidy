//
//  MacCategoryManagerView.swift
//  ShotTidyMac
//
//  Manage user-defined categories: list, create, edit, and delete.
//  Built-in categories are shown read-only for reference.
//

import SwiftUI
import SwiftData

struct MacCategoryManagerView: View {

    @Environment(\.modelContext) private var modelContext
    @Environment(\.dismiss) private var dismiss
    @Environment(CategoryStore.self) private var categoryStore

    @Query(sort: [SortDescriptor(\UserCategory.sortOrder), SortDescriptor(\UserCategory.createdAt)])
    private var userCategories: [UserCategory]

    @State private var editorCategory: UserCategory?
    @State private var showNewEditor = false
    @State private var deleteTarget: UserCategory?

    var body: some View {
        NavigationStack {
            List {
                customSection
                builtInSection
            }
            .listStyle(.inset)
            .navigationTitle("Categories")
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("Done") { dismiss() }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button {
                        showNewEditor = true
                    } label: {
                        Image(systemName: "plus")
                    }
                    .help("New Category")
                }
            }
            .sheet(isPresented: $showNewEditor) {
                MacCategoryEditorView()
            }
            .sheet(item: $editorCategory) { category in
                MacCategoryEditorView(existing: category)
            }
            .alert("Delete Category?", isPresented: Binding(
                get: { deleteTarget != nil },
                set: { if !$0 { deleteTarget = nil } }
            )) {
                Button("Delete", role: .destructive) {
                    if let target = deleteTarget { delete(target) }
                }
                Button("Cancel", role: .cancel) { deleteTarget = nil }
            } message: {
                Text("Items in this category will be kept but shown under \u{201C}Other\u{201D}. This cannot be undone.")
            }
            .frame(minWidth: 500, minHeight: 440)
        }
    }

    // MARK: - Custom categories section

    @ViewBuilder
    private var customSection: some View {
        Section {
            if userCategories.isEmpty {
                HStack(spacing: 12) {
                    Image(systemName: "folder.badge.plus")
                        .foregroundStyle(.secondary)
                    VStack(alignment: .leading, spacing: 2) {
                        Text("No custom categories yet")
                            .font(.subheadline)
                        Text("Click + to create one")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
                .padding(.vertical, 4)
            } else {
                ForEach(userCategories) { category in
                    Button {
                        editorCategory = category
                    } label: {
                        categoryRow(category.descriptor, showChevron: true)
                    }
                    .buttonStyle(.plain)
                    .contextMenu {
                        Button("Edit") { editorCategory = category }
                        Divider()
                        Button("Delete", role: .destructive) { deleteTarget = category }
                    }
                }
            }
        } header: {
            Text("My Categories")
        } footer: {
            Text("Custom categories are matched by AI when analyzing screenshots using the AI hint.")
        }
    }

    // MARK: - Built-in categories section

    private var builtInSection: some View {
        Section("Built-in") {
            ForEach(categoryStore.builtInDescriptors) { descriptor in
                categoryRow(descriptor, showChevron: false)
            }
        }
    }

    // MARK: - Row

    private func categoryRow(_ descriptor: CategoryDescriptor, showChevron: Bool) -> some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 8)
                    .fill(descriptor.color.opacity(0.15))
                    .frame(width: 34, height: 34)
                Image(systemName: descriptor.iconName)
                    .font(.system(size: 15, weight: .medium))
                    .foregroundStyle(descriptor.color)
            }
            Text(descriptor.name)
                .font(.subheadline)
                .foregroundStyle(.primary)
            Spacer()
            if showChevron {
                Image(systemName: "chevron.right")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(.tertiary)
            }
        }
        .padding(.vertical, 2)
    }

    // MARK: - Delete

    private func delete(_ category: UserCategory) {
        modelContext.delete(category)
        try? modelContext.save()
        categoryStore.reload()
        deleteTarget = nil
    }
}
